  ...
  <div class="login-container">
    <form class="form" @submit.prevent="login">
      <div class="login-head">
        ...
        <div class="login-sub">Aurora 博客管理后台</div>
      </div>
      ...
      <div class="inputForm">
        ...
        <input v-model.trim="loginForm.username" type="text" class="input" placeholder="请输入邮箱" />
      </div>
      ...
      <div class="inputForm">
        ...
        <input v-model="loginForm.password" :type="showPassword ? 'text' : 'password'" class="input" placeholder="请输入密码" />
        <svg
          class="eye-icon"
          :class="{ 'eye-icon--active': showPassword }"
          viewBox="0 0 576 512"
          height="1em"
      ...
      <button class="button-submit" type="submit" :disabled="loading">登 录</button>

      <p class="p line">或</p>

      <div class="flex-row">
        <button type="button" class="btn google" @click="thirdLogin('Google')">
          <svg version="1.1" width="20" height="20" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
        ...
        <button type="button" class="btn apple" @click="thirdLogin('Apple')">
...
export default {
  ...
  data: function () {
    return {
      loginForm: {
        username: localStorage.getItem('aurora_admin_username') || '',
        password: ''
      },
      remember: localStorage.getItem('aurora_admin_remember') === '1',
      showPassword: false,
      loading: false
    }
  },
  methods: {
    ...
    rememberUser() {
      if (this.remember) {
        localStorage.setItem('aurora_admin_username', this.loginForm.username)
        localStorage.setItem('aurora_admin_remember', '1')
      } else {
        localStorage.removeItem('aurora_admin_username')
        localStorage.removeItem('aurora_admin_remember')
      }
    },
    forgotPassword() {
    ...
    thirdLogin(provider) {
      this.$message.info(`${provider} 登录暂未开放`)
    }
  }
}
</script>
